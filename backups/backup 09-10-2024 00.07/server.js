...
// In je server.js of routes bestand
...
app.get('/inschrijvingen-beheren', (req, res) => {
    ...
    const inschrijvingen = []; // Dit moet de inschrijvingen zijn die je uit de database haalt

    res.render('inschrijvingen-beheren', {
        title: 'Inschrijvingen Beheren',
        inschrijvingen: inschrijvingen // Dit moet de daadwerkelijke data zijn
    });
});

// Route voor het beheren van diensten
app.get('/diensten', checkRole(['administrator']), (req, res) => {
    db.query('SELECT * FROM diensten', (err, results) => {
        if (err) {
            console.error('Fout bij het ophalen van diensten: ', err);
            return res.status(500).send('Fout bij het ophalen van diensten.');
        }
        res.render('diensten', { title: 'Dienstenbeheer', diensten: results });
    });
});

// Route voor het toevoegen van een nieuwe dienst
app.post('/diensten', checkRole(['administrator']), (req, res) => {
    const { activiteit, afdeling, soort_dienst, aanvang, einde } = req.body;

    db.query('INSERT INTO diensten (activiteit, afdeling, soort_dienst, aanvang, einde) VALUES (?, ?, ?, ?, ?)', [activiteit, afdeling, soort_dienst, aanvang, einde], (err) => {
        if (err) {
            console.error('Fout bij het toevoegen van dienst: ', err);
            return res.status(500).send('Fout bij het toevoegen van dienst.');
        }
        res.redirect('/diensten');
    });
});
