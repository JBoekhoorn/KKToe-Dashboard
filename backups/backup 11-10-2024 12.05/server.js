...
app.get('/openstaande-diensten', checkRole(['administrator']), (req, res) => {
    db.query('SELECT d.*, u.email FROM diensten d JOIN users u ON d.user_id = u.id', (err, results) => {
        if (err) {
            console.error('Fout bij het ophalen van diensten: ', err);
            return res.status(500).send('Fout bij het ophalen van diensten.');
        }
        res.render('openstaande-diensten', { title: 'Diensten', diensten: results });
    });
});

app.get('/openstaande-diensten', (req, res) => {
    db.query('SELECT * FROM diensten WHERE user_id IS NULL', (error, results) => {
        console.log('SQL-query uitgevoerd: SELECT * FROM diensten'); // Log de SQL-query
        console.log('Fout:', error); // Log eventuele fouten
        console.log('Resultaten:', results); // Log de resultaten van de query
        ...
        res.render('openstaande-diensten', { diensten: results });
    });
});
