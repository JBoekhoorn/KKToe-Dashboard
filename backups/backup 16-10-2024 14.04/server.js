...
app.get('/gebruikers', checkRole(['administrator']), async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Fout bij het ophalen van gebruikers.' });
    }
});
...
app.get('/admin/gebruikers', async (req, res) => {
    try {
        const [gebruikers] = await db.query('SELECT * FROM users'); // Haal gebruikers op uit de database
